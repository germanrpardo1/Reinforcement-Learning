...
class Board:
    def __init__(self):
        self.board = np.zeros((num_boards, rows, cols))
        self.reward = 0.0
        ...
        for j in range(num_boards):
            ...
            for i in range(cols):
                ...
                if sum(self.board[j, :, i]) == -3:
                    return -1
            # diagonal
            diag_sum1 = sum([self.board[j, i, i] for i in range(cols)])
            diag_sum2 = sum([self.board[j, i, cols - i - 1] for i in range(cols)])
...
class TreeNode():
    ...
    def get_value(self, c_puct):
        ...
        #np.sqrt(2 * np.log(self._parent._n_visits) / self._n_visits)
...
class MCTS():
    ...
    def get_move(self, env):
        ...
        return max(self._root._children.items(),
                   key=lambda act_node: act_node[1]._Q)[0]#n_visits
...
def rollout_policy_fn(board):
    ...
    # rollout randomly
    action_probs = np.random.rand(len(board.actions))
    return zip(board.actions, action_probs)

def policy_value_fn(board):
    """a function that takes in a state and outputs a list of (action, probability)
    tuples and a score for the state"""
    # return uniform probabilities and 0 score for pure MCTS
    action_probs = np.ones(len(board.actions))/len(board.actions)
    return zip(board.actions, action_probs), 0
